...
- `glab-axi mr close <iid> --auth-source native -R NAMESPACE/PROJECT --hostname HOST --expected-url URL --expected-source BRANCH --expected-target BRANCH --expected-head SHA --expected-state opened|closed [--format toon|json]` - Observe or request one exact reversible MR close transition.
- `glab-axi mr reopen <iid> --auth-source native -R NAMESPACE/PROJECT --hostname HOST --expected-url URL --expected-source BRANCH --expected-target BRANCH --expected-head SHA --expected-state opened|closed [--format toon|json]` - Observe or request one exact reversible MR reopen transition.

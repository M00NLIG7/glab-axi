...
- `gl-axi pipeline delete <ID> --auth-source native --hostname HOST -R PROJECT --expected-project-id ID --expected-url URL --confirm-delete-pipeline URL --expected-sha SHA --expected-ref REF --expected-status STATUS --expected-updated-at TIMESTAMP [--format toon|json]` - Guardedly delete one exact pipeline.
- `gl-axi release delete <TAG> --auth-source native --hostname HOST -R PROJECT --expected-project-id ID --expected-url URL --confirm-delete-release URL --expected-commit SHA --expected-created-at TIMESTAMP [--format toon|json]` - Guardedly delete one exact release.

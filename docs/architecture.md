...
bounded search, MR discussions, guarded issue edit, MR ensure, and guarded MR
merge, use internal fixed `glab api` routes.
...
## Guarded issue edit: exact-identity write

Product `issue edit` is the sole issue mutation. Its parser requires an explicit
host and nested project, canonical positive IID and issue URL, `opened` or
`closed` expected state, exact RFC 3339 `updated_at`, and at least one title,
description, or label request. It validates these values before target or
child discovery. Content comes only from descriptor-validated private regular
non-symlink files at the established title and description bounds. A title
cannot be blank; an empty description intentionally clears it.

Label arguments are repeatable but comma-free because the fixed GitLab update
fields are comma-delimited. The command rejects empty, duplicate,
case-colliding, overlapping, missing, ambiguous, or case-substituted names. It
consumes every bounded page of project and inherited labels, resolves requested
names to numeric label identities, repeats that complete lookup, and rejects
identity drift. The body uses only `add_labels` and `remove_labels`, never a
replacement label set, so unrelated labels remain untouched.
...
3. resolve requested labels twice when needed, then read the exact issue again
   as the final provider operation and reject stale evidence or snapshot drift;
4. return `preview` for `--dry-run` or `unchanged` for an exact no-op, with zero
   mutation in both cases;
5. otherwise write only changed fields through one private mode-0600 JSON file
   and one fixed PUT under a dedicated phase budget; and
6. after the attempt, issue exactly one bounded exact GET, detached from a
   canceled mutation context. Exact desired state yields `applied`, exact
   pre-write state yields `not_applied` for an uncertain response, a proven
   HTTP rejection remains a controlled error, and every
   other result is `ambiguous_issue_edit`.

Receipts include canonical project/issue identity, caller evidence, ordered
changed-field names, before/after values or SHA-256 evidence at output bounds,
label IDs, resulting `updated_at`, action, and outcome. The outer 45-second
write budget reserves 20 seconds for preflight, 15 seconds for the PUT, and 10
seconds for read-back. The approved v1 surface is pinned in
`contracts/issue-edit/v1.json`; creation, comments, state changes, assignment,
milestones, hierarchy, boards, approvals, credentials, and pipelines remain
outside it.
...
modify or integrate Firstmate. All other provider mutations outside guarded
issue edit and the two MR write contracts remain denied.
...
- Guarded issue-edit receipts hash text over 4 KiB and label sets over 100 names
  or 16 KiB while retaining exact byte/count evidence. Text hashes cover exact
  UTF-8 bytes; label hashes cover the compact JSON encoding of sorted names.

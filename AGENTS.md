...
- Product reads/login delegate only typed argv from `contracts/official-glab/v1.112.0/`; changing official glab requires a new evidence fixture and offline version/help gate. Never parse or export its credential store. Preserve the bounded PTY/ConPTY login boundary in `internal/delegate/glab`: all child streams must stay terminal-backed while the exact plaintext warning remains monitored.

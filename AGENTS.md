...
- `gl-axi` is the canonical product/module/command; `glab-axi` remains a tested executable alias with no removal date, while the repository URL and `glab-axi/*` wire/config/keyring identifiers stay stable for compatibility. The product is intentionally narrow: do not add commands or HTTP routes without a pinned consumer contract and fail-closed tests. Provider writes are limited to MR ensure and guarded immediate squash merge. Issue edit is validation-only because GitLab accepts no expected issue revision and only label names; non-no-op live requests refuse before PUT. Generic API, issue mutation, alternate/unguarded merge, close/comment, repository writes, and pipeline mutations stay out of scope.

...
func TestCapabilityFixturePinsEveryExecutableOperation(t *testing.T) {
	...
	for _, operation := range []Operation{OpSnippetUser, OpSnippetProject, OpSnippetList, OpSnippetView, OpSnippetFile, OpBoardList, OpBoardView, OpBoardIssues, OpWorkItemFields, OpWorkItemHierarchy, OpIssueList, OpIssueView, OpIssueEditProject, OpIssueEditView, OpIssueEditLabelList, OpMRList, OpMRView, OpMRDiff, OpMRChecks, OpMRDiscussions, OpMRDiscussionsTargetProject, OpMRDiscussionsSourceProject, OpPipelineList, OpPipelineView, OpJobList, OpJobView, OpJobTrace, OpReleaseList, OpReleaseView, OpRepoList, OpRepoView, OpLabelList, OpSearch, OpEnsureProject, OpEnsureList, OpEnsureCreate, OpEnsureUpdate, OpMergeProject, OpMergeMRView, OpMergeJobList, OpMergeBridgeList, OpMRMerge} {
...
func TestRequestBuilderRejectsInjectionBeforeExecution(t *testing.T) {
	for _, request := range []Request{
		...
		{Operation: Operation("issue-edit"), Host: "gitlab.com", Repo: "group/project", IID: 1, InputFile: filepath.Join(t.TempDir(), "payload.json")},

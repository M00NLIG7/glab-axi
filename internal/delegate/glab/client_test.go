...
func TestMutationHTTPClassificationRequiresPinnedStatusFraming(t *testing.T) {
	...
	unframed := classifyChildFailure(nil, []byte(`{"message":"403 Forbidden provider text"}`), cause, true, OpEnsureCreate)
	...
	framed := classifyChildFailure([]byte(`{"message":"403 Forbidden"}`), []byte(`glab: 403 Forbidden (HTTP 403)`), cause, true, OpEnsureCreate)
...
func TestMergeHTTPClassificationIsOperationSpecific(t *testing.T) {
	...
	for _, status := range []int{405, 406} {
		...
		body := []byte(fmt.Sprintf(`{"message":"%d rejected: provider-sentinel"}`, status))
		mergeErr := uxv1.AsError(classifyChildFailure(body, framed, cause, true, OpMRMerge))
		...
		ensureErr := uxv1.AsError(classifyChildFailure(body, framed, cause, true, OpEnsureCreate))
...
if [ "${GLAB_AXI_FAKE_MODE:-}" = "http-rejection" ]; then
  ...
  printf '{"message":"%s %s: %s"}\n' "${GLAB_AXI_FAKE_HTTP_STATUS}" "$reason" "${GLAB_AXI_FAKE_STDERR_SENTINEL:-synthetic}"

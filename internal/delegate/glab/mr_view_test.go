...
const mrViewTestHead = "0123456789012345678901234567890123456789"
...
func TestNormalizeMRViewResponseAcceptsOfficialAndRESTHeadShapes(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			body: `{"id":1011,"iid":11,"project_id":101,"source_project_id":101,"target_project_id":101,"diff_refs":{"base_sha":"1123456789012345678901234567890123456789","head_sha":"` + mrViewTestHead + `","start_sha":"2123456789012345678901234567890123456789"}}`,
		},
		{
			name: "prior REST response",
			body: `{"id":1011,"iid":11,"source_project_id":101,"target_project_id":101,"sha":"` + mrViewTestHead + `"}`,
		},
		{
			name: "matching dual proof",
			body: `{"id":1011,"iid":11,"project_id":101,"source_project_id":101,"target_project_id":101,"sha":"` + mrViewTestHead + `","diff_refs":{"head_sha":"` + mrViewTestHead + `"}}`,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if decoded["sha"] != mrViewTestHead || decoded["id"] != float64(1011) || decoded["iid"] != float64(11) {
...
func TestNormalizeMRViewResponseRejectsUnprovableIdentity(t *testing.T) {
	otherHead := "1123456789012345678901234567890123456789"
	for _, test := range []struct {
		name string
		body string
		code uxv1.Code
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if strings.Contains(err.Error(), mrViewTestHead) || strings.Contains(err.Error(), otherHead) {

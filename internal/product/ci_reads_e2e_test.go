...
case "$*" in
 ...
 'api --method GET --hostname gitlab.com projects/group%2Fproject/pipelines/55')
   cat "$CI_READ_FIXTURES/pipeline" ;;
 'api --method GET --hostname gitlab.com projects/group%2Fproject/pipelines/55/jobs?page=1&per_page=31&scope%5B%5D=failed')
   cat "$CI_READ_FIXTURES/jobs" ;;
 'api --method GET --hostname gitlab.com projects/group%2Fproject/jobs/9')
   cat "$CI_READ_FIXTURES/job" ;;
 'api --method GET --hostname gitlab.com projects/group%2Fproject/jobs/9/trace')
   printf 'last failure\n' ;;
...
`
			...
			run := func(args []string) (int, string, string) {
				t.Helper()
				cmd := exec.Command(binary, ciArgs(args...)...)
				cmd.Dir = dir
				cmd.Env = []string{"PATH=" + dir + ":/usr/bin:/bin", "HOME=" + dir, "GLAB_CONFIG_DIR=" + filepath.Join(dir, "config"), "CI_READ_RECORD=" + record, "CI_READ_FIXTURES=" + dir, "GITLAB_TOKEN=" + secret}
			...
			for _, args := range [][]string{
				{"pipeline", "list", "--ref", "main", "--status", "failed", "--source", "push", "--user", "alice", "--sha", ciTestSHA, "--fields", "iid,sha,web_url,updated_at"},
				...
				{"pipeline", "watch", "55", "--timeout", "3", "--interval", "1", "--ref", "main", "--sha", ciTestSHA},

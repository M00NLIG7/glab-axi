...
var definitions = append(issueWriteDefinitions(), append(append([]Definition{
	...
	{Path: []string{"pipeline", "list"}, Summary: "List project pipelines.", Usage: "gl-axi pipeline list [--ref REF] [--status STATUS] [--source SOURCE] [--user USERNAME] [--sha SHA] [--fields iid] [--web-base URL] [global flags]", Details: "Filters use exact GitLab values and are retained on every page. Username is a provider-side filter; the pinned pipeline list response omits user identity. Fields are additive: iid is opt-in, while existing SHA, URL and updated-at fields remain unchanged. A source is not a workflow resource.", RepoMode: RepoRequired, Flags: pipelineListFlags(), Schema: "pipeline-list", Backend: "official-glab"},
	{Path: []string{"pipeline", "view"}, Summary: "View one pipeline.", Usage: "gl-axi pipeline view <id> [--ref REF] [--sha SHA] [--jobs] [--job-id ID] [--job-status STATUS] [--trace | --trace-failed] [--web-base URL] [global flags]", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Flags: pipelineViewFlags(), Details: "Optional jobs and traces are selected reads, never complete jobs/bridges merge-check proof. Trace selection is capped at five jobs and 256 KiB per tail. CI reads cap cumulative provider bodies at 8 MiB, 100 requests, and final data at 2 MiB. No workflow or step alias and no full-log file escape.", Schema: "pipeline-view", Backend: "official-glab"},
	{Path: []string{"pipeline", "watch"}, Summary: "Watch one exact pipeline within a finite budget.", Usage: "gl-axi pipeline watch <id> [--timeout SECONDS] [--interval SECONDS] [--ref REF] [--sha SHA] [--web-base URL] [target/output flags]", Details: "Defaults: 30 seconds, 3-second interval. Hard bounds: 300 seconds, 100 requests, 8 MiB cumulative provider bodies, 64 KiB final data. Emits only a final result, never progress as success. Exit zero requires pipeline success; non-green, unknown/stale evidence, timeout, budget exhaustion, and caller cancellation fail truthfully. This is pipeline status, not merge readiness or complete job/bridge proof.", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Flags: pipelineWatchFlags(), Schema: "pipeline-watch", Backend: "official-glab", NoLimit: true},
	{Path: []string{"job", "list"}, Summary: "List jobs for one pipeline.", Usage: "gl-axi job list --pipeline-id ID [--job-id ID] [--status STATUS] [--web-base URL] [global flags]", RepoMode: RepoRequired, Flags: jobListFlags(), Schema: "job-list", Backend: "official-glab"},
	...
	{Path: []string{"job", "view"}, Summary: "View one CI/CD job.", Usage: "gl-axi job view <id> [--pipeline-id ID] [--web-base URL] [global flags]", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Flags: jobIdentityFlags(), Schema: "job-view", Backend: "official-glab"},
	{Path: []string{"job", "trace"}, Summary: "View a bounded, redacted tail of one job trace.", Usage: "gl-axi job trace <id> [--pipeline-id ID] [--web-base URL] [global flags]", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Flags: jobIdentityFlags(), Schema: "job-trace", Backend: "official-glab"},
	...
	{Path: []string{"repo", "create"}, Summary: "Create one empty project in an explicit namespace.", Details: "Provider metadata only: no source, template, clone, push, or merge-policy changes. Explicit account, namespace and visibility are required. See docs/project-administration.md for guards and residual races.", Usage: "gl-axi repo create --auth-source native -R NAMESPACE/PROJECT --hostname HOST --namespace-id ID --namespace-kind user|group --expected-user-id ID --expected-username NAME --visibility private|internal|public --allow-project-admin [--description-file FILE] [--format toon|json]", RepoMode: RepoRequired, Flags: repoAdminFlags("create"), Schema: "repo-admin", Backend: "native", Write: true, NoLimit: true, RequireExplicitHost: true, RequireExplicitRepo: true, NativeAuth: true, RequireNativeAuth: true},
	{Path: []string{"repo", "edit"}, Summary: "Edit exact project settings with caller prestate and drift checks.", Details: "Requires a private closed expected-state snapshot and explicit non-atomic acknowledgement. Supports description, visibility, default branch and GitLab issues/wiki access levels only. Never changes merge protections. See docs/project-administration.md.", Usage: "gl-axi repo edit --auth-source native -R NAMESPACE/PROJECT --hostname HOST --namespace-id ID --namespace-kind user|group --expected-user-id ID --expected-username NAME --expected-state-file FILE --allow-project-admin --accept-non-atomic [setting flags] [--format toon|json]", RepoMode: RepoRequired, Flags: repoAdminFlags("edit"), Schema: "repo-admin", Backend: "native", Write: true, NoLimit: true, RequireExplicitHost: true, RequireExplicitRepo: true, NativeAuth: true, RequireNativeAuth: true},
	{Path: []string{"repo", "fork"}, Summary: "Request one asynchronous project fork into an explicit namespace.", Details: "Provider-only fork. Accepted/in-progress is not ready. Optional polling is bounded; no clone, remote or push effects. Never retry an ambiguous mutation blindly. See docs/project-administration.md.", Usage: "gl-axi repo fork --auth-source native -R SOURCE/PROJECT --hostname HOST --expected-source-id ID --destination NAMESPACE/PROJECT --namespace-id ID --namespace-kind user|group --expected-user-id ID --expected-username NAME --visibility private|internal|public --allow-project-admin [--description-file FILE] [--wait-seconds 0..20] [--format toon|json]", RepoMode: RepoRequired, Flags: repoAdminFlags("fork"), Schema: "repo-admin", Backend: "native", Write: true, NoLimit: true, RequireExplicitHost: true, RequireExplicitRepo: true, NativeAuth: true, RequireNativeAuth: true},
	...
	{Path: []string{"repo", "view"}, Summary: "View a project/repository.", Usage: "gl-axi repo view [namespace/project] [--admin-snapshot --auth-source native] [global flags]", RepoMode: RepoOptional, MaxPositions: 1, Flags: []FlagDefinition{{Name: "--admin-snapshot", Boolean: true, Description: "Include complete edit prestate using --auth-source native and explicit --repo/--hostname. Store admin_snapshot in a private file for repo edit."}}, Schema: "repo-view", Backend: "official-glab", NativeAuth: true},
...
}, append(planningDefinitions(), variableDefinitions()...)...), deletionDefinitions()...)...)
...
func TopHelp() string {
	...
	out.WriteString("\nBackends:\n  default product operations and human login use pinned official glab 1.112.0 (816e3a52);\n  exact glab-axi/v1 automation remains a standalone native backend;\n  declared downloads, issue-write commands, CI variable commands, guarded resource deletion and project administration require explicit --auth-source native, with no profile fallback.\n")
	out.WriteString("\nSecurity boundary:\n  typed nonblank issue create/comment, MR ensure, guarded squash merge, guarded CI variable set/delete, opted-in board issues and explicit project create/edit/fork may write;\n  board issues may initialize ordering and shift sibling positions;\n  issue close/reopen refuse transitions; only already-matching state observations succeed;\n  blank issue creation is temporarily unavailable;\n  issue edit validates and previews but refuses live mutation because GitLab has no enforceable issue revision;\n  explicit native deletion is limited to exact issues, pipelines, releases and snippets;\n  pipeline deletion requires separate acknowledgment of possible surviving-child cancellation;\n  release deletion requires separate acknowledgment of possible CI/CD Catalog unpublication;\n  label deletion remains disabled because provider ID-or-title fallback can select a sibling;\n  no generic API, approve, MR comment/reply/resolve/close/reopen, MR or repository deletion,\n  label-resource or MR-label mutation, repository writes outside guarded create/edit/fork, other release or pipeline/job writes,\n  or alternate merge strategy. CI variable values are never displayed.\n")
	return out.String()
}

...
var definitions = append(issueWriteDefinitions(), append(append([]Definition{
	...
	{Path: []string{"issue", "discussions"}, Summary: "View bounded issue comments and threaded notes.", Details: "Includes individual and system notes, in provider order. The limit counts threads.\nExact issue/project identity is rechecked around pagination. Empty means no visible discussions, not denied access.\nNo comment, reply, or resolution mutation is exposed.", Usage: "gl-axi issue discussions <iid> [global flags]", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Schema: "issue-discussions", Backend: "official-glab"},
	...
	{Path: []string{"mr", "approvals"}, Summary: "View assigned reviewers and current GitLab approval state.", Details: "Assigned reviewers are not submitted reviews. Use mr discussions for inline/threaded notes.\nThe approved boolean is provider-reported and edition-dependent; absent state is unknown, never inferred from counts.\n403/404 approval responses are unavailable, not zero approvals; tier stays unknown. Approval rules are not exposed.\nThe limit bounds each user array. Identity/base/head are rechecked; this is observational evidence, not merge authority.", Usage: "gl-axi mr approvals <iid> [--expected-head SHA] [global flags]", RepoMode: RepoRequired, Positionals: 1, MaxPositions: 1, Flags: []FlagDefinition{{Name: "--expected-head", Value: "SHA", Description: "Optionally require an exact lowercase 40- or 64-hex head."}}, Schema: "mr-approvals", Backend: "official-glab"},

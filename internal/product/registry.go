...
var definitions = append(issueWriteDefinitions(), append(append([]Definition{
	...
	{Path: []string{"repo", "list"}, Summary: "Discover repositories with explicit user/group ownership.", Details: "Unfiltered discovery preserves the official-profile default. User owners and groups are distinct.\nFiltered host discovery lists accessible projects. Group discovery excludes shared projects; descendants are opt-in.\nLanguage means uses the language, not GitHub primary language. No group language filter.\nClone URLs are opt-in metadata only, bound to the selected host and project.", Usage: "gl-axi repo list [USER] [--owner USER | --group FULL_PATH] [global flags]", RepoMode: RepoNone, MaxPositions: 1, Flags: discoveryFlags(), Schema: "repo-list", Backend: "official-glab"},
	{Path: []string{"repo", "view"}, Summary: "View a project/repository.", Usage: "gl-axi repo view [namespace/project] [--fields clone_urls] [global flags]", RepoMode: RepoOptional, MaxPositions: 1, Flags: []FlagDefinition{{Name: "--fields", Value: "clone_urls", Description: "Add host/project-bound HTTPS and SSH clone metadata; no local clone."}}, Schema: "repo-view", Backend: "official-glab"},

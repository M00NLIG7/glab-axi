...
var definitions = append(issueWriteDefinitions(), append(append([]Definition{
	...
	mrWriteDefinition("comment", true),
	mrWriteDefinition("note", true),
	mrWriteDefinition("close", false),
	mrWriteDefinition("reopen", false),
	...
	{Path: []string{"mr", "ensure"}, Summary: "Create or update exactly one matching open merge request.", Details: "Optional --assignee-id and --reviewer-id (repeatable, at most 20), --milestone-id, and --draft select creation metadata and require --auth-source native. The complete native operation uses one selected environment/keyring identity, which may differ from the official profile.\nWith these selectors an existing match must already have all selected metadata and exact title/body; no replacement PUT is attempted.\nNumeric IDs are explicit provider identities, not username or milestone-name lookups.", Usage: "gl-axi mr ensure --source BRANCH --target BRANCH --title-file FILE --description-file FILE [--auth-source native] [global flags]", RepoMode: RepoRequired, Flags: ensureFlags(), Schema: "mr-ensure", Backend: "official-glab", Write: true, NativeAuth: true},
	{Path: []string{"mr", "create-or-update"}, Summary: "Alias for bounded MR ensure semantics.", Details: "Supports the same creation-only --assignee-id, --reviewer-id, --milestone-id, and --draft selectors as mr ensure, requiring --auth-source native. Default title/body ensure retains official-profile behavior.\nExisting selected metadata and content must already match; collections are never replaced.", Usage: "gl-axi mr create-or-update --source BRANCH --target BRANCH --title-file FILE --description-file FILE [--auth-source native] [global flags]", RepoMode: RepoRequired, Flags: ensureFlags(), Schema: "mr-ensure", Backend: "official-glab", Write: true, NativeAuth: true},
...
func mrWriteDefinition(action string, note bool) Definition {
	...
	summary := "Observe an already-matching MR state; " + action + " transitions are temporarily refused."
	if note {
...
func ensureFlags() []FlagDefinition {
	return []FlagDefinition{
		{Name: "--assignee-id", Value: "ID", Description: "Numeric assignee ID for creation; repeatable, at most 20. Existing selected collections must already match.", Repeatable: true},
		{Name: "--reviewer-id", Value: "ID", Description: "Numeric reviewer ID for creation; repeatable, at most 20. Existing selected collections must already match.", Repeatable: true},
		{Name: "--milestone-id", Value: "ID", Description: "Numeric milestone ID for creation; existing milestone must already match."},
		{Name: "--draft", Description: "Create with Draft: title prefix; refuse an existing non-draft match.", Boolean: true},

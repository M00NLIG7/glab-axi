...
var definitions = append(issueWriteDefinitions(), append(append([]Definition{
	...
	{Path: []string{"mr", "ensure"}, Summary: "Create or update exactly one matching open merge request.", Details: "Optional --assignee-id and --reviewer-id (repeatable, at most 20), --milestone-id, and --draft select creation metadata.\nWith these selectors an existing match must already have all selected metadata and exact title/body; no replacement PUT is attempted.\nNumeric IDs are explicit provider identities, not username or milestone-name lookups.", Usage: "gl-axi mr ensure --source BRANCH --target BRANCH --title-file FILE --description-file FILE [global flags]", RepoMode: RepoRequired, Flags: ensureFlags(), Schema: "mr-ensure", Backend: "official-glab", Write: true},
	{Path: []string{"mr", "create-or-update"}, Summary: "Alias for bounded MR ensure semantics.", Details: "Supports the same creation-only --assignee-id, --reviewer-id, --milestone-id, and --draft selectors as mr ensure.\nExisting selected metadata and content must already match; collections are never replaced.", Usage: "gl-axi mr create-or-update --source BRANCH --target BRANCH --title-file FILE --description-file FILE [global flags]", RepoMode: RepoRequired, Flags: ensureFlags(), Schema: "mr-ensure", Backend: "official-glab", Write: true},
...
func mrWriteDefinition(action string, note bool) Definition {
	usage := "gl-axi mr " + action + " <iid> -R NAMESPACE/PROJECT --hostname HOST --expected-url URL --expected-source BRANCH --expected-target BRANCH --expected-head SHA --expected-state opened|closed"
	summary := "Observe or request one exact reversible MR " + action + " transition."
	if note {

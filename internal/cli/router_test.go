...
func TestCanonicalAndCompatibilityIssueEditHelpMatch(t *testing.T) {
	for _, program := range []string{"gl-axi", "glab-axi"} {
		...
		for _, required := range []string{program + " issue edit <iid>", "--expected-url URL", "--expected-state STATE", "--expected-updated-at TIMESTAMP", "--add-label NAME", "--remove-label NAME", "--dry-run", "one PUT sends only changed fields", "cannot enforce an atomic expected revision", "concurrent edits or label renames can race"} {

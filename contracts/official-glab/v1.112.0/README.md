...
schema. Guarded issue edit pins exact project, issue, label-catalog, and one
private-input PUT route. Guarded merge pins four fixed reads and one fixed PUT;
the PUT consumes only a private four-key JSON file, is invoked once, and is
never delegated through interactive `glab mr merge` behavior.
...
ensure, exact-MR-view normalization, guarded issue-edit, and guarded-merge
requests with synthetic credentials; it never contacts a live GitLab API.
